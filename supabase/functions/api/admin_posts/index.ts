...
import { jsonError, jsonSuccess, withRateLimitHeaders, withRequestIdHeaders, jsonErrorWithId, problemJson } from '../_shared/responses.ts';
import {
...
export default async function (req: Request) {
  ...
  try {
    if (req.method === 'GET' && !postId) {
      ...
      if (fetchError) return jsonErrorWithId('Failed to load posts', 500, {}, rateHeaders, reqId);
    ...
    if (req.method === 'POST' && !postId) {
      ...
      if (!parse.success) {
        return problemJson(
          'https://errors.careercompass.ug/invalid_payload',
          'Invalid payload',
          400,
      ...
      if (insertError) {
        ...
        return jsonErrorWithId('Failed to create post', 500, {}, rateHeaders, reqId);
      }
    ...
    if (req.method === 'PUT' && postId) {
      ...
      if (payload.content !== undefined) updates.content = sanitizeHtml(payload.content);
      ...
      if (updateError) {
        ...
        return jsonErrorWithId('Failed to update post', 500, {}, rateHeaders, reqId);
      }
    ...
    if (req.method === 'DELETE' && postId) {
      const { error: deleteError } = await supabase
        ...
        .delete()
        .eq('id', postId);
      if (deleteError) return jsonErrorWithId('Failed to delete post', 500, {}, rateHeaders, reqId);
