...
function removeExisting() {
  document
    .querySelectorAll<HTMLScriptElement>(
      'script[data-jobposting-json-ld="true"]',
    )
    ...
    const items = jobs.map((j) => {
      ...
      const applyUrl =
        ...
        (j.application_method === "email" && j.application_email
          ? `mailto:${j.application_email}`
          : undefined);
      const jobLocationType = j.region?.toLowerCase().includes("remote")
        ? "TELECOMMUTE"
        : undefined;
      ...
      return {
        ...
        applicationContact: j.application_email
          ? { "@type": "ContactPoint", email: j.application_email }
          : undefined,
